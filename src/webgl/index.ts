...
const BEND_DECAY = 0.86;
const BEND_REST = 0.05;
...
export const webglRenderer = (
  ...
  install: (ctx: EngineFeatureContext) => {
    ...
    const tick = (): void => {
      ...
      bend *= BEND_DECAY;
      const settled =
        !ctx.isDragging() && !ctx.isAnimating() && Math.abs(bend) < BEND_REST;
      if (settled) {
        bend = 0;
        paint();
        running = false;
    ...
    ctx.addTeardown(ctx.on("dragstart", wake));
